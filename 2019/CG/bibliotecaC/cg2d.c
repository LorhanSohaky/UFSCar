...
int DrawLine(point *p1, point *p2, window *win, viewport *port,
...
int DrawObject(object *ob, window *win, viewport *port, bufferdevice *dev,
  ...
  for (i = 0; i < ob->numbers_of_points; i++) {
    p1 = SetPoint(ob->points[i].x, ob->points[i].y, ob->points[i].w,
                  ob->points[i].color);
...
object *TransObj(object *ob, matrix *m) {
  ...
  for (i = 0; i < ob->numbers_of_points; i++) {
    p = SetPoint(ob->points[i].x, ob->points[i].y, ob->points[i].w,
                 ob->points[i].color);
    pt = LinearTransf(m, p);
